...
def ipadapter_execute(model,
    ...
    cond_comp = None
    cond_alt = None
    img_cond_embeds = None
    img_uncond_embeds = None
    ...
    if image is not None:
        ...
        # Handle composition image if provided
    ...
    if is_faceid and is_plus:
        ...
        if img_comp_cond_embeds is not None:
            comp_cond, _ = ipa.get_image_embeds(img_comp_cond_embeds, img_uncond_embeds, encode_batch_size)
            cond_alt = { 3: comp_cond.to(device, dtype=dtype) }

    cond = cond.to(device, dtype=dtype)
    uncond = uncond.to(device, dtype=dtype)

    # Clean up memory
    del img_cond_embeds, img_uncond_embeds, img_comp_cond_embeds, face_cond_embeds, image_negative, image_composition
...
class IPAdapterFaceID(IPAdapterAdvanced):
    ...
    def apply_ipadapter(self, model, ipadapter, image, weight, weight_faceidv2, weight_type, combine_embeds, start_at, end_at, embeds_scaling, image_negative=None, attn_mask=None, clip_vision=None, insightface=None):
        ipa_args = {
            "image": image,
            "image_negative": image_negative,
            "image_composition": None,  # Explicitly set to None for FaceID
            "weight": weight,
            "weight_faceidv2": weight_faceidv2,
            "weight_type": weight_type,
            "combine_embeds": combine_embeds,
